...
from aind_data_schema_models.base import AindGeneric, AindGenericType, AindModel, AwareDatetimeWithDefault
from aind_data_schema_models.coordinates import RelativePosition, Size3d
...
from aind_data_schema_models.reagent import Reagent
...
class Objective(Device):
    ...
    def validate_other(self):
        ...
        if self.immersion == ImmersionMedium.OTHER and self.notes is None:
...
class HarpDevice(DAQDevice):
    ...
    def validate_other(self):
        ...
        if self.data_interface == DataInterface.OTHER and self.notes is None:
            raise ValueError(
...
class AdditionalImagingDevice(Device):
    ...
    def validate_other(self):
        ...
        if self.imaging_device_type == ImagingDeviceType.OTHER and self.notes is None:
            raise ValueError(
