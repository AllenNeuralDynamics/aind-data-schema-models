...
def create_literal_model(obj:dict, base_model:BaseModel, discriminator:str, field_handlers:Optional[dict]=None, class_module=None):
...
def create_literal_class(
    ...
    all_models = tuple(
        create_literal_model(obj=obj, base_model=base_model, discriminator=discriminator, field_handlers=field_handlers, class_module=class_module) 
        for obj in objects
    )
